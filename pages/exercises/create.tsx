...
const CreateExercisePage: NextPage = () => {
	...
	const toggleArrayItem = (field: 'targetMuscles' | 'secondaryMuscles' | 'exerciseEquipment', value: string) => {
		const currentArray = exerciseData[field] || [];
		const index = currentArray.indexOf(value as any);
		if (index > -1) {
			setExerciseData({
				...exerciseData,
				[field]: currentArray.filter((_, i) => i !== index),
			...
			setExerciseData({
				...exerciseData,
				[field]: [...currentArray, value as any],
			});
		}
	};
                                ...
                                <Stack spacing={2}>
                                    ...
                                    <Stack spacing={1}>
                                        {exerciseData.exerciseWarnings?.map((warning, index) => (
                                            <Box key={index} sx={{ display: 'flex', alignItems: 'center', gap: 1, p: 1, bgcolor: 'error.light', borderRadius: 1 }}>
							...
							<Stack direction="row" spacing={2} alignItems="center">
								...
								{exerciseData.exerciseImage && (
									<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
										<img
											src={`${REACT_APP_API_URL}/${exerciseData.exerciseImage}`}
											alt="Exercise"
											style={{ width: 100, height: 100, objectFit: 'cover', borderRadius: 4 }}
										/>
										<IconButton size="small" onClick={() => setExerciseData({ ...exerciseData, exerciseImage: undefined })}>
											<CloseIcon />
										</IconButton>
									</Box>
								)}
							...
							<Stack direction="row" spacing={2} alignItems="center">
								...
								{exerciseData.exerciseVideo && (
									<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
										...
										<IconButton size="small" onClick={() => setExerciseData({ ...exerciseData, exerciseVideo: undefined })}>
											<CloseIcon />
										</IconButton>
									</Box>
								)}
						...
						<Box>
							...
							<Stack direction="row" spacing={2} alignItems="center">
								...
								{exerciseData.exerciseGif && (
									<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
										<img
											src={`${REACT_APP_API_URL}/${exerciseData.exerciseGif}`}
											alt="Exercise GIF"
											style={{ width: 100, height: 100, objectFit: 'cover', borderRadius: 4 }}
										/>
										<IconButton size="small" onClick={() => setExerciseData({ ...exerciseData, exerciseGif: undefined })}>
											<CloseIcon />
										</IconButton>
									</Box>
								)}
